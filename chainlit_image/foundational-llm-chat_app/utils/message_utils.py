...
def create_content(text: Optional[str], images: Optional[List[Dict[str, Any]]], 
                  docs: Optional[List[Dict[str, Any]]]) -> List[Dict[str, Any]]:
    """
...
def create_image_content(images: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for image in images:
        try:
            ...
            logger.debug(f"Successfully encoded image: {image['name']} ({image['type']})")
...
def extract_and_process_prompt(prompt_object: Dict[str, Any]) -> str:
    ...
    try:
        ...
        if 'variants' in prompt_object and prompt_object['variants']:
            ...
            if 'templateConfiguration' in variant and 'text' in variant['templateConfiguration']:
                ...
                if 'inputVariables' in variant['templateConfiguration']['text']:
                    ...
                    for var in input_variables:
                        ...
                        if var_name == 'TODAY':
                            ...
                            logger.warning(f"Unsupported variable in prompt: {var_name}")
            ...
            logger.warning(f"No variants found in prompt object: {prompt_object.keys()}")
            return ""
    except Exception as e:
