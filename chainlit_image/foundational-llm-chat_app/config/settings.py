...
class Settings:
    ...
    def load_content_limits() -> Dict[str, Any]:
        ...
        return {
            "max_chars": None if (not os.getenv("MAX_CHARACHERS") or os.getenv("MAX_CHARACHERS") == "None") 
                        else int(os.getenv("MAX_CHARACHERS")),
            "max_content_size_mb": None if (not os.getenv("MAX_CONTENT_SIZE_MB") or os.getenv("MAX_CONTENT_SIZE_MB") == "None") 
    ...
    def load_data_layer_config() -> Dict[str, Optional[str]]:
        ...
        return {
            "dynamodb_table": None if (not os.getenv("DYNAMODB_DATA_LAYER_NAME") or os.getenv("DYNAMODB_DATA_LAYER_NAME") == "None") 
                             else str(os.getenv("DYNAMODB_DATA_LAYER_NAME")),
            "s3_bucket": None if (not os.getenv("S3_DATA_LAYER_NAME") or os.getenv("S3_DATA_LAYER_NAME") == "None") 
