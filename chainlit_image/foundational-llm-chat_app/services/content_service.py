...
class ContentService:
    ...
    def __init__(self, max_chars: Optional[int] = None, max_size_mb: Optional[float] = None):
        """
    ...
    def verify_content(self, text: Optional[str], images: List[Dict[str, Any]], 
                      docs: List[Dict[str, Any]]) -> bool:
        """
        ...
        if text and self.max_chars and len(text) > self.max_chars:
            logger.warning(f"Text exceeds maximum length: {len(text)} > {self.max_chars}")
            return False
                ...
                try:
                    with Image.open(image["path"]) as img:
                        ...
                        if self.max_size_mb and size > self.max_size_mb:
                            logger.warning(f"Image exceeds maximum size: {size} MB > {self.max_size_mb} MB")
                            return False
        ...
        for doc in docs:
            if not os.path.isfile(doc["path"]):
                ...
                try:
                    ...
                    if self.max_size_mb and size > self.max_size_mb:
                        logger.warning(f"Document exceeds maximum size: {size} MB > {self.max_size_mb} MB")
                        return False
                except Exception as e:
    ...
    def split_message_contents(self, message: Any, model_id: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], List[Dict[str, Any]]]:
        """
                ...
                if element.type == "file" or element.type == "image":
                    ...
                    if file_type.startswith("image/") or element.type == "image":
                        images.append({
                            "path": file_path,
                            "name": file_name,
                            "type": file_type or "image/png",  # Default to PNG if no MIME type
                            "size": file_size
                        })
                        logger.debug(f"Added image: {file_name}")
                    # Check if the file is a document
                    elif file_type in ["application/pdf", "text/plain", "text/html", "text/markdown",
                                      "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                                      "application/msword", "application/vnd.ms-excel",
                                      "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                      "text/csv"] or file_name.lower().endswith(('.txt', '.pdf', '.doc', '.docx', '.xls', '.xlsx', '.csv', '.html', '.htm', '.md')):
                        docs.append({
                            "path": file_path,
                            "name": file_name,
                            "type": file_type or "text/plain",  # Default to text/plain if no MIME type
                            "size": file_size
                        })
        ...
        logger.debug(f"Split message contents: {len(images)} images, {len(docs)} documents, {len(other_files)} other files")
        return images, docs, other_files
        
    def delete_contents(self, contents: List[Dict[str, Any]], force: bool = False) -> None:
        """
